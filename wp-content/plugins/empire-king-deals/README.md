...
Location and order destination metadata remain allowlisted. No outbound domains are stored here: the Avenue H theme renders local `/order-now/` links with the `ekb_cat` destination and allowlisted campaign attribution. Future product routing can extend this metadata contract. Only `/deals/` owns public SEO; records/categories have no public singles, archives, REST exposure, navigation entries, or sitemap entries.
