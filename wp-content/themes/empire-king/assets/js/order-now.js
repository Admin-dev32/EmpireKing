document.addEventListener('DOMContentLoaded', () => {
	...
	const products = document.querySelector('[data-order-now-products]');
	...
	if (rail && cards.length) rail.addEventListener('click', (event) => {
		...
		if (categoryStart < rail.scrollLeft || categoryEnd > rail.scrollLeft + rail.clientWidth) {
			rail.scrollTo({
				left: Math.max(0, categoryStart - (rail.clientWidth - category.offsetWidth) / 2),
		...
		if (products && !reducedMotion.matches) {
			products.classList.remove('is-category-transitioning');
