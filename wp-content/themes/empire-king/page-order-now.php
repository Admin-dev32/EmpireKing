...
$requested_category = isset( $_GET['ekb_cat'] ) && is_scalar( $_GET['ekb_cat'] ) ? sanitize_title( wp_unslash( $_GET['ekb_cat'] ) ) : '';
...
$order_now_url = get_permalink( get_queried_object_id() );
get_header();
?>
<div class="ek-order-now"<?php if ( $product_background ) : ?> style="--ek-product-background: url('<?php echo esc_url( $product_background ); ?>')"<?php endif; ?>>
	...
	<section class="ek-order-now__menu-surface" aria-label="Order menu categories">
		<?php if ( $categories ) : ?><nav class="ek-order-now__categories" aria-label="Product categories" data-order-now-categories><?php foreach ( $categories as $category ) : $category_url = add_query_arg( 'ekb_cat', $category->slug, $order_now_url ); ?><a class="ek-order-now__category<?php echo $active_category === $category->slug ? ' is-active' : ''; ?>" href="<?php echo esc_url( $category_url ); ?>" data-order-now-category="<?php echo esc_attr( $category->slug ); ?>"><?php echo esc_html( $category->name ); ?></a><?php endforeach; ?></nav><?php endif; ?>
