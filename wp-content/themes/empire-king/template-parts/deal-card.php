...
$order_url = empire_king_get_local_order_url( $deal['destination'], true );
$linked_product_id = function_exists( 'ek_deals_product_id' ) ? ek_deals_product_id( $deal['product_id'] ?? 0 ) : 0;
if ( $linked_product_id ) {
	$order_url = add_query_arg( 'product_id', $linked_product_id, $order_url );
}
?>
<article class="ek-deals__card<?php echo $lead ? ' ek-deals__card--lead' : ''; ?><?php echo $image_id ? ' ek-deals__card--has-art' : ''; ?>" id="<?php echo esc_attr( $deal['anchor'] ); ?>">
	...
	<div class="ek-deals__copy">
		...
		<a class="ek-deals__order" href="<?php echo esc_url( $order_url ); ?>">Order This Deal <span aria-hidden="true">&rarr;</span></a>
	</div>
</article>
