...
- Variation IDs are kept only for one unambiguous exact attribute map under the matched parent. New combinations are created. Obsolete variations become private (excluded from WooCommerce's published child lookup), with their IDs/history retained. Banana Split becomes simple at 6.75 and any obsolete variations are made private. Old published parent products outside this package become draft and hidden, never deleted.
